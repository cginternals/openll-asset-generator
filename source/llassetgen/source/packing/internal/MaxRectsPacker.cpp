...
// Compares against range (excluding)
template <typename T>
...
{
    const auto rectMin = rect.position;
    const auto rectMax = rect.position + rect.size;
    const auto bboxMin = bbox.position;
    const auto bboxMax = bbox.position + bbox.size;
...
PackingSizeType bssfScore(const Rect<PackingSizeType>& free, const Rect<PackingSizeType>& toBePlaced)
{
    ...
    const auto remainder = toBePlaced.size - free.size;
...
bool MaxRectsPacker::inputSortingComparator(const Rect<PackingSizeType>& rect1, const Rect<PackingSizeType>& rect2)
{
...
void MaxRectsPacker::cropRects(const Rect<PackingSizeType>& placedRect)
{
    ...
    for (size_t i = 0; i < rectsToCrop;)
    {
        ...
        const auto freeRectCopy = freeList[i];
