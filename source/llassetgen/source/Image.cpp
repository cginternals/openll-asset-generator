...
Image Image::view(Vec2<size_t> outerMin, Vec2<size_t> outerMax, size_t padding) const
{
...
template <typename pixelType>
void Image::setPixel(Vec2<size_t> pos, const pixelType in) const
{
    assert(isValid(pos));
...
template LLASSETGEN_API void Image::fillRect<float>(const Vec2<size_t> & _min, const Vec2<size_t> & _max, float in) const;
template LLASSETGEN_API void Image::fillRect<uint32_t>(const Vec2<size_t> & _min, const Vec2<size_t> & _max, uint32_t in) const;
template LLASSETGEN_API void Image::fillRect<uint16_t>(const Vec2<size_t> & _min, const Vec2<size_t> & _max, uint16_t in) const;
template LLASSETGEN_API void Image::fillRect<uint8_t>(const Vec2<size_t> & _min, const Vec2<size_t> & _max, uint8_t in) const;
template <typename pixelType>
void Image::fillRect(const Vec2<size_t> & _min, const Vec2<size_t> & _max, const pixelType in) const
{
...
Image::Image(const std::string& filepath, uint8_t _bitDepth)
{
    ...
    for (size_t y = 0; y < getHeight(); y++)
    {
        for (size_t x = 0; x < getWidth(); x++)
        {
            const auto pixel = multi_channel_data.get()[y * png_stride * channels + x * png_bitDepth / 8 * channels];
            const auto reducedPixel = reduceBitDepth(pixel, png_bitDepth, bitDepth);
...
size_t Image::divisiblePadding(const size_t size, const size_t padding, const size_t divisor)
{
    const size_t paddedSize = size + 2 * padding;
    const size_t moduloPadding = (divisor - (paddedSize % divisor)) % divisor;
